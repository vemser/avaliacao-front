...
export const CadastrarFeedback = () => {
  ...
  const { pegarModulo, modulo, pegarModuloPorFiltro } = useContext(ModuloContext);
  ...
  const { register, handleSubmit, formState: { errors }, control } = useForm<IFeedbackCadastro>({
          ...
          <FormControl sx={{ width: "100%" }}>
            <Controller control={control} name="modulo" render={({ field: { onChange } }) => (
              <Autocomplete sx={{ width: "100%" }}
                multiple disablePortal onInputChange={(event, value) => {
                  filtroDebounce(value, pegarModuloPorFiltro, pegarModulo, `&nome=${value}`)
                }} onChange={(e, values) => { if (values.length === 0) setModuloSelecionado([]); setModuloSelecionado(values.map((value) => value.id)); handleChangeModulo(); }} id="modulo" noOptionsText=""
                isOptionEqualToValue={(option, value) => option.label === value.label}
                options={modulo ? modulo.elementos.map((modulos) => ({ label: `${modulos.nome}`, id: modulos.idModulo })) : []} renderOption={(props, option) => (<li {...props} key={option.id}>{option.label}</li>)}
